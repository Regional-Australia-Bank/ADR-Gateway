...
export const GetRegisterAccessToken = async (cert:ClientCertificateInjector, params: {
  ...
  config
  let client_id = params.SoftwareProductConfig.ProductId; //Should be software product ID, not brand ID  //Using default [0] to suffice until Dr G has a product ID in context
  let options = {
      method: <"POST">"POST",
