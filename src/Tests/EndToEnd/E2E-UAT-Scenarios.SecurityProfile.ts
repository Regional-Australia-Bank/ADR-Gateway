...
export const Tests = ((env: E2ETestEnvironment) => {
    ...
    describe('Security Profile', async () => {
        ...
        describe('Arrangement management', async () => {    
            ...
            Scenario($ => it.apply(this, $('wip DELETE consent')), undefined, 'Delete consent using the Dr. G API (at DH arrangement endpoint)')
                ...
                .Then(async ctx => {
                    ...
                    expect(revocationResult.config.data).to.match(/cdr_arrangement_id=/);  // change back to cdr_arrangment_id from cdr_arrangement_jwt
        ...
        describe('Data Recipient Endpoints', async () => {
            ...
            Scenario($ => it.apply(this,$('wip Revocation (cdr_arrangement_jwt)')), undefined, 'Data recipient honours valid revocation request')
                .Given('New Authorization')
                .Precondition("DH private JWKS available", ctx => {
                    if (!ctx.environment.Config.SystemUnderTest.DhRevokePrivateJwks) {
                        throw "No Dh Private JWKS"
                ...
                .When(DoRequest, async (ctx) => {
                    ...
                    const arrangementId = (await ctx.GetResult(NewGatewayConsent)).consent!.arrangementId
            ...
            // testing if we send both cdr_arrangement_id it need to verify cdr_arrangement_jwt matched
            Scenario($ => it.apply(this,$('wip Revocation (cdr_arrangement_jwt with cdr_arrange_id')), undefined, 'Data recipient honours valid revocation request')
                .Given('New Authorization')
                .Precondition("DH private JWKS available", ctx => {
                    if (!ctx.environment.Config.SystemUnderTest.DhRevokePrivateJwks) {
                        throw "No Dh Private JWKS"
            ...
            // test when cdr_arrangement_id does not match with cdr_arrangement_jwt
            // testing if we send both cdr_arrangement_id it need to verify cdr_arrangement_jwt matched
            Scenario($ => it.apply(this,$('wip Revocation (Not match cdr_arrangement_jwt with cdr_arrange_id')), undefined, 'Data recipient honours valid revocation request')
                .Given('New Authorization')
                .Precondition("DH private JWKS available", ctx => {
                    if (!ctx.environment.Config.SystemUnderTest.DhRevokePrivateJwks) {
                        throw "No Dh Private JWKS"
                ...
                .When(DoRequest, async (ctx) => {
                    ...
                    let options = DoRequest.Options({
                        ...
                        data: qs.stringify({
                            cdr_arrangement_jwt: await CreateCDRArrangementJWTAssertion(arrangementId),
                            cdr_arrangement_id: arrangementId + 'some rndom string'
                        }),
                        headers: {
                            "Authorization": "Bearer " + await CreateDhBearerAuthJwt(url)
                        },
                ...
                .Then(async ctx => {
                    let revocationResult = await ctx.GetResult(DoRequest, "Revocation")
                    expect(revocationResult.response.status).to.equal(400);
                    expect(revocationResult.response.data.errors[0].code).to.equal("urn:au-cds:error:cds-banking:DeleteArrangement/NotMatched");
                    expect(revocationResult.response.data.errors[0].title).to.equal("cdr_arrangement_id not matched");
            ...
            Scenario($ => it.apply(this, $('Unknown cdr_arrangement_id')), undefined, 'Get 422 for unknown cdr_arrangement_id')
                ...
                .Then(async ctx => {
                    ...
                    expect(revocationResult.response.data.errors[0].code).to.equal("urn:au-cds:error:cds-banking:DeleteArrangement/NotFound");
                    expect(revocationResult.response.data.errors[0].title).to.equal("Consent not found");

                }, 600)

            Scenario($ => it.apply(this, $('invalid request cdr_arrangement_id')), undefined, 'Get 400 for invalid request')
                .Given('New Authorization')
                .Precondition("DH private JWKS available", ctx => {
                    if (!ctx.environment.Config.SystemUnderTest.DhRevokePrivateJwks) {
                        throw "No Dh Private JWKS"
                ...
                .When(DoRequest, async (ctx) => {
                    ...
                    let options = DoRequest.Options({
                        method: "POST",
                        data: qs.stringify({
                            bad_arrangement_id: arrangementId
                        }),
                        url,
                        headers: {
                            "Authorization": "Bearer " + await CreateDhBearerAuthJwt(url)
                ...
                .Then(async ctx => {
                    let revocationResult = await ctx.GetResult(DoRequest, "Revocation")
                    expect(revocationResult.response.status).to.equal(400);
                    expect(revocationResult.response.data.errors[0].code).to.equal("urn:au-cds:error:cds-banking:DeleteArrangement/RequestValidation");
                    expect(revocationResult.response.data.errors[0].title).to.equal("Invalid request");
